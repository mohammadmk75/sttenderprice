...
if pb > 0 and beta > 0 and gama > 0:
    ...
    if cont_count < 3:
        ...
        for i in range(1, cont_count + 1):
            ...
            estimate_pi_price = format_number_input(f"Enter {company_name}'s Tender Price (IRR):" , key=f"price_{i}")
        ...
        if two_step:
            impact_factor = st.number_input("Enter the Common Impact Factor (i):", min_value=0.0)
        ...
        if st.button("Evaluate"):
            ...
            if acceptable_companies:
                ...
                st.write(f"Unacceptable xi threshold: {unacceptable_threshold:.4f}")
                ...
                if acceptable_companies:
                    ...
                    st.write(f"Acceptable Range: ({lower_bound:.4f}, {upper_bound:.4f})")
